...
class SpatialAttnProcessor2_0(torch.nn.Module):
    r"""
    Attention processor for IP-Adapaper for PyTorch 2.0.
    Args:
        hidden_size (`int`):
        ...
        layer_tag (`str`, optional):
            Unique identifier for the attention layer.
    """
    def __init__(self, hidden_size=None, cross_attention_dim=None, id_length=4, 
                 n_characters=1, slot_offset=0, device="cuda", dtype=torch.float16, layer_tag=None):
        super().__init__()
        ...
        self.total_length = id_length * n_characters + 1
        self.id_bank = {}
    ...
    def __call__(
        ...
        attention_mask=None,
        temb=None):
        layer_step = self.step_dict.get(self.layer_tag, 0)
        if self.write:
            # Allocate the list once per diffusion step
            if layer_step not in self.id_bank:
                self.id_bank[layer_step] = [None] * (self.n_char * self.id_length)
            start = self.slot
            end = self.slot + self.id_length
            # Store this block of reference images
            self.id_bank[layer_step][start:end] = hidden_states[:self.id_length]
        else:
            if layer_step not in self.id_bank:  # Guard for first story step
                encoder_hidden_states = hidden_states
            else:
                try:
                    refs = [t.to(hidden_states) for t in self.id_bank[layer_step]]
                    encoder_hidden_states = torch.cat(
                        refs[:self.slot] +                    # refs before
                        [hidden_states[:1]] +                 # the “current” slice
                        refs[self.slot:] +                    # refs after
                        [hidden_states[1:]], dim=0)
                except RuntimeError as e:
                    print(f"Shape mismatch in torch.cat: "
                          f"refs={[t.shape for t in refs]}, "
                          f"hidden_states[:1]={hidden_states[:1].shape}, "
                          f"hidden_states[1:]={hidden_states[1:].shape}")
                    raise e
        if layer_step < 5:
            ...
            if random_number > rand_num:
                if not self.write:
                    if hidden_states.shape[1] == 32 * 32:
                        ...
                        attention_mask = self.mask256[self.mask256.shape[0] // self.total_length * self.id_length:]
                    else:
        ...
        if self.attn_count == self.total_count:
            self.attn_count = 0
            self.cur_step += 1
            self.step_dict[self.layer_tag] = self.step_dict.get(self.layer_tag, 0) + 1
            self.mask256, self.mask1024, self.mask4096 = cal_attn_mask(
                self.total_length, self.id_length, self.sa16, self.sa32, self.sa64,
    ...
    for ind, string in enumerate(generate_prompt_arr):
        ...
        if start != -1 and end != -1:
            ...
            if key in character_dict:
                raise gr.Error("duplicate character description: " + key)
