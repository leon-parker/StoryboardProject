...
# === Prompts ===
prompts = [
    "A young child walks into a well-lit bathroom, realistic photography, consistent character",
    "The same child picks up a colorful toothbrush and toothpaste, realistic style, consistent character",
    "The same child squeezes toothpaste onto the toothbrush over the sink, consistent character",
    "The same child brushes their teeth in front of a clean bathroom mirror, consistent character", 
    "The same child rinses with water and smiles in the mirror, consistent character"
]
...
try:
    ...
    # === Method 1: Try IP-Adapter with different weights ===
    try:
        ...
        # Try different IP-Adapter weights
        ip_adapter_configs = [
        ...
        if ip_adapter_loaded:
            # Generate remaining frames with IP-Adapter
            ...
            for i, prompt in enumerate(prompts[1:], start=2):
                ...
                # Keep first image as reference for consistency
                # reference_image = image  # Uncomment to use previous frame as reference
        else:
        ...
        # === Method 2: Img2Img approach ===
        # Generate first image with text-to-image
        ...
        # Generate remaining frames using img2img
        ...
        for i, prompt in enumerate(prompts[1:], start=2):
            ...
            result = img2img_pipe(
                ...
                negative_prompt=negative_prompt,
                image=current_image,
                strength=0.4,  # Lower strength for more consistency
                num_inference_steps=30,
                guidance_scale=7.5,
                generator=generator
            )
